...
import { X, CheckCircle, Loader2, Plus, Trash2, UploadCloud, Image as ImageIcon } from "lucide-react";
...
export default function TemplateModal({ isOpen, onClose, template, onSave }) {
    ...
    const tabs = [
        ...
        { id: "tables", label: "Materials & Financials" },
        { id: "lists", label: "Benefits & Terms" },
    ];
                    ...
                    <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 bg-slate-50/30">
                        {activeTab === "general" && (
                            <div className="space-y-5">
                                ...
                                <div><label className="block text-xs font-semibold text-slate-700 mb-1">Introduction Text</label><textarea rows={3} value={formData.intro || ""} onChange={e => setFormData(p => ({...p, intro: e.target.value}))} className="w-full px-4 py-2 border rounded-lg text-sm" /></div>
                            </div>
                        )}
                        ...
                        {activeTab === "howItWorks" && (
                            <div className="space-y-5">
                                <div>
                                    <label className="block text-sm font-semibold text-slate-700 mb-1">Paragraph 1 (System Operation)</label>
                                ...
                                <div>
                                    <label className="block text-sm font-semibold text-slate-700 mb-1">Paragraph 2 (Power Utilization)</label>
                                ...
                                <div>
                                    <label className="block text-sm font-semibold text-slate-700 mb-1">Paragraph 3 (Testing & Handover)</label>
                        ...
                        {activeTab === "images" && (
                            <div className="grid grid-cols-2 gap-8">
                                <div>
                                    ...
                                    <div className="border-2 border-dashed border-slate-300 rounded-xl p-6 flex flex-col items-center justify-center text-center hover:bg-slate-50 transition-colors relative h-48">
                                        {formData.diagramImage ? (
                                            <img src={formData.diagramImage} alt="Diagram" className="max-h-full max-w-full object-contain" />
                                        ) : (
                                            <><UploadCloud size={32} className="text-slate-400 mb-2" /><span className="text-sm text-slate-500 font-medium">Click to upload diagram</span></>
                                        )}
                                ...
                                <div>
                                    ...
                                    <div className="border-2 border-dashed border-slate-300 rounded-xl p-6 flex flex-col items-center justify-center text-center hover:bg-slate-50 transition-colors relative h-[244px]">
                                        {formData.singleLineImage ? (
                                            <img src={formData.singleLineImage} alt="Single Line" className="max-h-full max-w-full object-contain" />
                                        ) : (
                                            <><UploadCloud size={32} className="text-slate-400 mb-2" /><span className="text-sm text-slate-500 font-medium">Click to upload single line diagram</span></>
                                        )}
                                        ...
                                        {uploadingImage === "singleLineImage" && <div className="absolute inset-0 bg-white/80 flex items-center justify-center"><Loader2 className="animate-spin text-blue-500" /></div>}
                                    </div>
                                </div>
                            </div>
                        )}
                        ...
                        {activeTab === "tables" && (
                            <div className="space-y-8">
                                ...
                                <div>
                                    <div className="flex justify-between items-center mb-2">
                                        <label className="block text-sm font-bold text-slate-700">Financial Rows (Desc | Rate | Total | Disc | Final)</label>
                                        <button type="button" onClick={() => addNestedRow("financialRows", 5)} className="text-xs flex items-center gap-1 text-blue-600 bg-blue-50 px-2 py-1 rounded hover:bg-blue-100"><Plus size={12}/> Add Row</button>
                                    </div>
                                    <div className="space-y-2">
                                        {(formData.financialRows || []).map((row, rIdx) => (
                                            <div key={rIdx} className="flex gap-2 items-center">
                                                {[0, 1, 2, 3, 4].map(cIdx => (
                                                    <input key={cIdx} type="text" value={row[cIdx] || ""} onChange={(e) => handleNestedChange("financialRows", rIdx, cIdx, e.target.value)} className={`px-3 py-1.5 border rounded-lg text-sm ${cIdx === 0 ? 'flex-[2]' : 'flex-[1]'}`} placeholder={`Col ${cIdx+1}`} />
                                                ))}
                                                <button type="button" onClick={() => removeNestedRow("financialRows", rIdx)} className="p-1.5 text-red-400 hover:text-red-600 hover:bg-red-50 rounded"><Trash2 size={16}/></button>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div><label className="block text-xs font-semibold text-slate-700 mb-1">Amount Label (Total Number)</label><input type="text" value={formData.amountLabel || ""} onChange={e => setFormData(p => ({...p, amountLabel: e.target.value}))} className="w-full px-4 py-2 border rounded-lg text-sm" /></div>
                                    <div><label className="block text-xs font-semibold text-slate-700 mb-1">Amount Words</label><input type="text" value={formData.amountWords || ""} onChange={e => setFormData(p => ({...p, amountWords: e.target.value}))} className="w-full px-4 py-2 border rounded-lg text-sm" /></div>
                                </div>
                            </div>
                        )}
